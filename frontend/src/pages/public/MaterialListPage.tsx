...
export function MaterialListPage() {
  ...
  return (
    <PageLayout variant="public">
      <h1 className="font-minecraft text-2xl text-mc-grass mb-6">素材精选</h1>
      {loading && <LoadingState />}
      {!loading && !data?.items.length && <EmptyState text="暂无素材" />}
      <div className="grid grid-cols-3 gap-3">
        {data?.items.map((m) => (
          <Link key={m.materialId} to={`/materials/${m.materialId}`} className="panel-mc p-3 hover:border-mc-grass text-center">
            <p className="text-sm text-text-primary">{m.title}</p>
            <p className="text-xs text-text-muted mt-1">{m.authorName}</p>
            {m.featured && <span className="text-xs text-mc-gold">★ 精选</span>}
          </Link>
        ))}
      </div>
