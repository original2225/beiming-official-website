...
export function AnnouncementListPage() {
  ...
  return (
    <PageLayout variant="public">
      <h1 className="font-minecraft text-2xl text-mc-grass mb-6">公告</h1>
      {loading && <LoadingState />}
      {!loading && !data?.items.length && <EmptyState text="暂无公告" />}
      {data?.items.map((item) => (
        <Link key={item.contentId} to={`/announcements/${item.contentId}`} className="block panel-mc p-3 mb-2 hover:border-mc-grass">
