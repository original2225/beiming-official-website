...
export function ChangelogPage() {
  ...
  return (
    <PageLayout variant="public">
      ...
      {data?.items.map((c) => (
        <Link key={c.releaseId} to={`/changelog/${c.releaseId}`} className="block panel-ink p-3 mb-2 hover:border-indigo">
          ...
          {c.tags.length > 0 && <div className="mt-1 flex gap-1">{c.tags.map((t) => <span key={t} className="chip-ink">{t}</span>)}</div>}
        </Link>
      ))}
