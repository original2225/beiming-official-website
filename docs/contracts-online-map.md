...
所有接口默认使用 `/api/v1/online-map` 前缀。当前运行入口为 `portal-core-service:8134`，历史独立入口 `online-map-service:8121` 已退役且不得恢复。自检摘要必须返回当前运行端口 `8134`，并可在生产化缺口或模块装配摘要中保留历史端口 `8121` 作为原端口记录。
