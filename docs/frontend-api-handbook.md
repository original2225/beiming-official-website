...
本地联调当前仍可走网关回滚入口 `http://127.0.0.1:8125`。第三十二轮外部入口切流本地演练下，官网后端单服务目标入口仍是 `http://127.0.0.1:8135`，`externalEntrypointLocalCutoverRehearsalStatus=PASS_LOCAL_REHEARSAL_NOT_PRODUCTION` 只表示样板、候选入口、当前入口、回滚入口和 32 个 smoke 目标已经通过本地演练验证。只有存在可提交的前端、反向代理或部署入口配置时，才允许把 API base URL 指向该入口。`externalEntrypointConfigSamplePrecheckStatus=BLOCKED_BY_CUTOVER_SAMPLE_NOT_APPLIED` 表示仓库已经提供外部入口切流样板和回滚口径，但样板尚未应用到生产；`externalEntrypointCutoverPrecheckStatus=BLOCKED_BY_EXTERNAL_ENTRYPOINT_CONFIG_NOT_PROVIDED` 表示仓库未提供真实外部入口配置；`productionAuditSinkPrecheckStatus=BLOCKED_BY_PERSISTENT_AUDIT_SINK_NOT_CONFIGURED` 表示真实持久化审计 sink 尚未配置，前端或管理后台只能把它展示为生产替换阻塞项，不能当成审计已接入。生产集中配置前置门禁、生产化硬化门禁和五个 core 的退役前置证据不改变前端接入方式，也不表示真实集中配置 provider、生产 profile、敏感配置外置或持久化审计已经接入。所有业务路径保持 `/api/v1/...` 原样，例如登录请求是 `POST /api/v1/auth/login`，不是 `/api/v1/gateway/auth/login`，也不是 `/api/v1/unified-backend/auth/login`。本仓库没有真实前端或外部代理配置，所以这里只给外部配置接入口径，不代表生产流量已切换。
