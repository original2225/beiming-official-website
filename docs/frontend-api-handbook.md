...
本地联调当前仍可走网关回滚入口 `http://127.0.0.1:8125`。第二十九轮外部入口切换适配与观测准备下，官网后端单服务目标入口仍是 `http://127.0.0.1:8135`，但只有存在可提交的前端、反向代理或部署入口配置时，才允许把 API base URL 指向该入口。`externalEntrypointCutoverPrecheckStatus=BLOCKED_BY_EXTERNAL_ENTRYPOINT_CONFIG_NOT_PROVIDED` 表示仓库未提供真实外部入口配置；生产集中配置前置门禁、生产化硬化门禁和五个 core 的退役前置证据不改变前端接入方式，也不表示真实集中配置 provider、生产 profile、敏感配置外置或持久化审计已经接入。所有业务路径保持 `/api/v1/...` 原样，例如登录请求是 `POST /api/v1/auth/login`，不是 `/api/v1/gateway/auth/login`，也不是 `/api/v1/unified-backend/auth/login`。本仓库没有真实前端或外部代理配置，所以这里只给外部配置接入口径，不代表生产流量已切换。
