...
| `api-gateway` | 8125 | 回滚入口、路由表、上游健康、请求日志和业务请求转发对照。 | `docs/contracts-api-gateway.md` |
...
第四十三轮 `api-gateway-service` 受控退役预检不会改变前端业务路径。运维页可以读取 `apiGatewayControlledRetirementStatus`、`apiGatewayControlledRetirementChecks` 和 `apiGatewayControlledRetirementEvidence`，并在 `apiGatewayControlledRetirementStatus=BLOCKED_BY_API_GATEWAY_RETIREMENT_RECEIPT_NOT_PROVIDED` 时展示为真实退役收据和删除清单未提供。该状态引用的正式样板是 `docs/unified-backend-api-gateway-retirement-receipt-sample.json`，只证明本地门禁存在，不证明生产入口已经切到 `unified-backend-service:8135`。联调 `http://127.0.0.1:8135` 时仍保持 `/api/v1/**` 原路径。

第四十四轮 `api-gateway-service` 外部退役证据接收与删除审批门禁也不会改变前端业务路径。运维页可以读取 `apiGatewayExternalRetirementEvidenceStatus`、`apiGatewayExternalRetirementEvidenceChecks` 和 `apiGatewayExternalRetirementEvidence`，并在 `apiGatewayExternalRetirementEvidenceStatus=BLOCKED_BY_EXTERNAL_API_GATEWAY_RETIREMENT_EVIDENCE_NOT_PROVIDED` 时展示为真实外部退役证据和用户删除清单确认未提供。该状态引用的正式样板是 `docs/unified-backend-api-gateway-external-retirement-evidence-sample.json`，只证明本地证据接收门禁存在，不证明生产入口已经切到 `unified-backend-service:8135`。联调 `http://127.0.0.1:8135` 时仍保持 `/api/v1/**` 原路径。

第四十六轮真实生产入口切流证据闭环门禁也不会改变前端业务路径。运维页可以读取 `realProductionEntrypointCutoverStatus`、`realProductionEntrypointCutoverChecks` 和 `realProductionEntrypointCutoverEvidence`，并在 `realProductionEntrypointCutoverStatus=BLOCKED_BY_REAL_PRODUCTION_ENTRYPOINT_CUTOVER_EVIDENCE_NOT_PROVIDED` 时展示为真实生产入口切流证据未提供。该状态引用的正式样板是 `docs/unified-backend-real-production-entrypoint-cutover-evidence-sample.json`，只证明本地切流证据接收门禁存在，不证明生产入口已经切到 `unified-backend-service:8135`。即使切流证据样板完整，`oldApiGatewayRetirementAllowed` 仍保持 `false`。联调 `http://127.0.0.1:8135` 时仍保持 `/api/v1/**` 原路径。

第五十一轮外部入口与切流证据接收门禁也不会改变前端业务路径。运维页可以读取 `externalEntrypointCutoverEvidenceIntakeStatus`、`externalEntrypointCutoverEvidenceIntakeChecks` 和 `externalEntrypointCutoverEvidenceIntakeEvidence`，并在 `externalEntrypointCutoverEvidenceIntakeStatus=BLOCKED_BY_EXTERNAL_ENTRYPOINT_CUTOVER_EVIDENCE_NOT_PROVIDED` 时展示为外部入口切流证据未提供。该状态引用的正式样板是 `docs/unified-backend-external-entrypoint-cutover-evidence-intake-sample.json`，只证明本地证据接收格式存在，不证明前端入口、反向代理 upstream、部署入口、回滚入口、灰度权重、观测或审批已经在生产应用。`readyForProduction=false`、`readyToReplaceGateway=false` 和 `oldApiGatewayRetirementAllowed=false` 必须照常展示为阻塞态。联调 `http://127.0.0.1:8135` 时仍保持 `/api/v1/**` 原路径。

第四十七轮本地开发态 `api-gateway-service` 入口退役不会改变前端业务路径。运维页可以读取 `localApiGatewayEntrypointRetirementStatus`、`localApiGatewayEntrypointRetirementChecks` 和 `localApiGatewayEntrypointRetirementEvidence`，当前 `localApiGatewayEntrypointRetirementStatus=PASS_LOCAL_API_GATEWAY_ENTRYPOINT_RETIRED_UNIFIED_GATEWAY_APIS_PRESERVED` 时展示为本地旧网关 Maven 入口已退役。联调目标仍是 `unified-backend-service:8135`，业务路径仍保持 `/api/v1/**`。

本轮五个 core 独立 Maven 入口退役也不会改变前端业务路径。运维页可以读取 `coreEntrypointRetirementPrecheckStatus=PASS_LOCAL_CORE_MAVEN_ENTRYPOINTS_RETIRED_UNIFIED_MODULES_PRESERVED`，展示为本地独立启动入口已退役、模块源码仍由 unified 装配。`readyToRetireBusinessCore=false`、`readyToRetireAdmissionCore=false`、`readyToRetireEngagementCore=false`、`readyToRetireOpsCore=false` 和 `readyToRetirePortalCore=false` 仍表示真实生产退役未闭环。前端不得提示业务模块被删除，不得改成 `/api/v1/unified-backend/<module>`。
