...
第四十三轮 `api-gateway-service` 受控退役预检不会改变前端业务路径。运维页可以读取 `apiGatewayControlledRetirementStatus`、`apiGatewayControlledRetirementChecks` 和 `apiGatewayControlledRetirementEvidence`，并在 `apiGatewayControlledRetirementStatus=BLOCKED_BY_API_GATEWAY_RETIREMENT_RECEIPT_NOT_PROVIDED` 时展示为真实退役收据和删除清单未提供。该状态引用的正式样板是 `docs/unified-backend-api-gateway-retirement-receipt-sample.json`，只证明本地门禁存在，不证明生产入口已经切到 `unified-backend-service:8135`，也不证明 `api-gateway-service` 已退役。没有真实退役收据前，`VITE_API_BASE_URL` 默认仍可指向 `http://127.0.0.1:8125`；联调 `http://127.0.0.1:8135` 时仍保持 `/api/v1/**` 原路径。五个 core 退役状态必须继续按 `readyToRetireBusinessCore=false`、`readyToRetireAdmissionCore=false`、`readyToRetireEngagementCore=false`、`readyToRetireOpsCore=false` 和 `readyToRetirePortalCore=false` 展示，不得提示可删除。

第四十四轮 `api-gateway-service` 外部退役证据接收与删除审批门禁也不会改变前端业务路径。运维页可以读取 `apiGatewayExternalRetirementEvidenceStatus`、`apiGatewayExternalRetirementEvidenceChecks` 和 `apiGatewayExternalRetirementEvidence`，并在 `apiGatewayExternalRetirementEvidenceStatus=BLOCKED_BY_EXTERNAL_API_GATEWAY_RETIREMENT_EVIDENCE_NOT_PROVIDED` 时展示为真实外部退役证据和用户删除清单确认未提供。该状态引用的正式样板是 `docs/unified-backend-api-gateway-external-retirement-evidence-sample.json`，只证明本地证据接收门禁存在，不证明生产入口已经切到 `unified-backend-service:8135`，不证明 `api-gateway-service` 已删除，也不批准五个 core。没有真实外部证据前，`VITE_API_BASE_URL` 默认仍可指向 `http://127.0.0.1:8125`；联调 `http://127.0.0.1:8135` 时仍保持 `/api/v1/**` 原路径。五个 core 退役状态必须继续按 `readyToRetireBusinessCore=false`、`readyToRetireAdmissionCore=false`、`readyToRetireEngagementCore=false`、`readyToRetireOpsCore=false` 和 `readyToRetirePortalCore=false` 展示，不得提示可删除。
