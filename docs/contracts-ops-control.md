...
本文档是 `ops-control` 模块的正式 API 契约。后续前端运维控制台、`admin` 聚合、外部节点执行器和其他业务模块只能通过本文档定义的接口读取或管理服务器运维控制面，不能直接读取或修改 `ops-control` 数据，也不能把真实服务器命令、文件系统、Docker、虚拟机、Minecraft 实例或终端执行能力塞进其他服务。
