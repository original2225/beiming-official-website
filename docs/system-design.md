...
北冥官网从零重新设计。当前仓库不继承旧代码，但正式吸收旧需求文档 v5.0 中已经确认的业务范围，并把同伴之前推进过的后台资源与服务器管理能力纳入系统边界。
...
旧记录显示，项目曾经出现过两条不同方向。早期方案偏 Java 8、Spring Boot 2.7、Nacos、MySQL、多模块微服务；后续同伴推进过 Beiming Console 形态，包含 React 前端、API 网关、Spring Boot auth-service、resource-service、Go node-daemon、PostgreSQL 和本地启动脚本。

当前仓库不把任何一条旧技术路线直接当成最终结论。正式设计只吸收已经被验证过的架构经验：认证服务应独立成边界，网关或后端入口应负责统一路由和认证转发，资源服务应和后台运维控制区分，Go node-daemon 适合承接服务器本机操作。

旧 auth-service 中已经出现过注册、登录、当前用户、退出、会话校验、用户列表和用户修改能力。重新实现时可以把这些作为 auth 的基础用例，但不要照搬 `SUPER_ADMIN/ADMIN/MEMBER` 角色名，也不要默认沿用随机 session token、JWT、PostgreSQL 或 MySQL。认证方案在实现前单独确认，最终必须兼容本设计中的 `OWNER`、`ADMIN`、`HELPER`、`USER` 和细粒度能力点。

旧网关记录中的 `/api/auth/**`、`/api/users/**`、`/api/cloud/**` 和认证校验转发思路可以保留为参考，但当前正式接口路径需要在各模块实现前重新定义并固化到接口契约中。
...
后端语言和框架需要在开始实现 P0 前最终确认。账号、官网、社区等业务模块适合 Java 或 Node 系后端。节点守护进程适合 Go，因为它需要管理系统资源、进程、容器、文件和日志流。旧记录中的 Java 21、Spring Boot 3、PostgreSQL 方向和早期 Java 8、Spring Boot 2、MySQL、Nacos 方向都只是候选经验，不能在未确认时写死到实现里。
