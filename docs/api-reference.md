...
`behaviorCoverageByModule` 必须按 community、activity、calendar 和 changelog 返回完整行为契约覆盖进度。当前 community 64 个和 changelog 23 个业务方法路由已迁入 `engagement-core-service` 完整行为契约测试，待补数均为 `0`；activity 和 calendar 仍待补，待补路由数分别为 `41` 和 `21`。代表路由测试、后台自检测试和路由签名测试不得计入完整行为契约覆盖。

### 验收口径

`engagement-core` API 文档按 `docs/contracts-engagement-core.md` 独立存在，并由 `.local-docs/tests-engagement-core.md` 记录本地测试闭环。`mvn -f backend/engagement-core-service/pom.xml test` 必须覆盖三个自有接口、149 个第三批业务 `METHOD path` 路由签名、community 和 changelog 完整行为契约迁入、旧服务不恢复保护和后续行为契约缺口公开。
