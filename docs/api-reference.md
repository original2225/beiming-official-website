...
验收时必须确认 `backend/unified-backend-service` 可独立测试，真实 HTTP 演练、路由漂移扫描、后端单服务准备 evidence 和最终单服务后端侧收束 evidence 有自动化覆盖，当前 7 个生产后端 Maven 入口和候选入口回归通过，`node-daemon` 不进入候选入口源码扫描和 component scan，已退役旧服务入口没有恢复，生产源码危险删除命令、真实节点执行、终端、RCON、Docker 执行和备份恢复写入扫描无命中。第二十一轮完成后只能说明候选入口具备后端唯一应用服务切换预演证据，不能说明已经完成生产流量切换、前端或外部代理切换、旧入口退役或 `node-daemon` 合并。
