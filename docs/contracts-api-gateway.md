...
公开健康检查无需认证。网关自有后台接口需要 `Authorization: Bearer <token>`。P0 本地实现允许 `owner-token`、`admin-token` 和 `helper-token` 访问只读后台自检、路由、健康和请求日志接口；`user-token` 返回 `42001`；缺失或格式错误返回公共认证错误码。未来接入真实认证上下文时，必须先更新本文档和自动化测试。
...
| `routesTotal` | integer | 是 | 路由总数，P0 为 `24`。 |
...
`serviceKey` 不存在返回 `43000`。连接失败返回 HTTP `200` 且 `data.status` 为 `DOWN`，并记录 `lastErrorCode: 46210`。超时返回 HTTP `200` 且 `data.status` 为 `TIMEOUT`，并记录 `lastErrorCode: 46211`。健康刷新自身不能因为单个上游失败返回 5xx，除非网关内部状态不可用。
...
上游返回 2xx、4xx 或 5xx 时，网关默认透传。上游不可连接返回 HTTP `502` 和错误码 `46210`。上游超时返回 HTTP `504` 和错误码 `46211`。未知路径返回 HTTP `404` 和错误码 `46200`。不支持的方法返回 HTTP `405` 和错误码 `46201`。
...
单个上游不可用只影响该上游对应请求和健康摘要，不影响其他路由。上游不可连接返回 `46210`，上游超时返回 `46211`，上游非 HTTP 响应返回 `46212`。网关不能把上游失败转成空成功响应，不能改写上游业务错误为成功，不能重试非幂等请求。
