...
成功响应 HTTP `200`。`data` 必须包含 `readyForProduction=false`、`readyToReplaceGateway=false`、`readyToRetireBusinessCore=false`、`readyToRetireAdmissionCore=false`、`readyToRetireEngagementCore=false`、`readyToRetireOpsCore=false`、`readyToRetirePortalCore=false`、`currentProductionEntrypointsTotal=7`、`candidateEntrypointsTotal=1`、`checks`、`lastHttpSmokeStatus`、`productionBlockers`、`productionSwitchReadinessStatus`、`productionSwitchChecks`、`centralConfigPrecheckStatus`、`centralConfigPrecheckChecks`、`persistentAuditPrecheckStatus`、`persistentAuditPrecheckChecks`、`realHttpRehearsalPrecheckStatus`、`realHttpRehearsalPrecheckChecks`、`routeDriftPrecheckStatus`、`routeDriftPrecheckChecks`、`rollbackWindowPrecheckStatus`、`rollbackWindowPrecheckChecks`、`rollbackWindowEvidence`、`entrypointSwitchPrecheckStatus`、`entrypointSwitchPrecheckChecks`、`entrypointSwitchEvidence`、`productionTrafficCanaryEvidence`、`backendSingleServicePrecheckStatus`、`backendSingleServicePrecheckChecks`、`backendSingleServiceEvidence`、`replacementDecision` 和 `generatedAt`。
...
为后端侧单服务生产入口准备，readiness 必须额外暴露后端单服务预检摘要。第十六轮开始，`backendSingleServicePrecheckStatus` 可以为 `PASS`，只表示 `unified-backend-service:8135` 已具备后端唯一候选入口的可运行、可配置、可审计、可回滚和可验证形态；它不得表示前端、外部代理或生产流量已经切换。`backendSingleServicePrecheckChecks` 每项必须包含 `check`、`status`、`detail` 和 `requiredForReplacement`。其中 `UNIFIED_BACKEND_COVERS_BACKEND_ENTRYPOINT_APIS`、`ALL_NON_NODE_DAEMON_ROUTES_IN_PROCESS`、`PATH_AUTH_ENVELOPE_AND_ERROR_CODES_PRESERVED`、`REAL_HTTP_REHEARSAL_PASSED`、`ROUTE_DRIFT_SCAN_PASSED`、`SENSITIVE_FIELD_SCAN_PASSED`、`ROLLBACK_WINDOW_EVIDENCE_COMPLETED`、`CURRENT_ENTRYPOINTS_PRESERVED_AS_ROLLBACK`、`CURRENT_ENTRYPOINT_REGRESSION_REQUIRED`、`NODE_DAEMON_EXTERNAL_BOUNDARY` 和 `BACKEND_SINGLE_SERVICE_EVIDENCE_RECORDED` 必须为 `PASS`；`FRONTEND_ENTRYPOINT_SWITCH_IMPLEMENTED`、`EXTERNAL_PROXY_SWITCH_IMPLEMENTED`、`OLD_ENTRYPOINT_RETIREMENT_APPROVED`、`CENTRAL_CONFIG_PROVIDER_CONNECTED` 和 `PERSISTENT_AUDIT_SINK_CONNECTED` 必须为 `BLOCKED`。`backendSingleServiceEvidence` 必须包含 `candidateEntrypoint=unified-backend:8135`、`currentGatewayRollbackTarget=api-gateway:8125`、`currentCoreRollbackTargets=["business-core:8130","admission-core:8131","engagement-core:8132","ops-core:8133","portal-core:8134"]`、`nodeDaemonDisposition=KEEP_EXTERNAL`、`businessPathsRemainUnchanged=true`、`inProcessRoutesTotal=25`、`httpFallbackRoutesTotal=0`、`currentProductionEntrypointsPreserved=true`、`frontendEntrypointSwitched=false`、`externalProxySwitched=false`、`trafficSwitchApplied=false`、`oldEntrypointRetirementApproved=false`、`backendSingleServiceCandidateReady=true` 和 `remainingBlockers`。`remainingBlockers` 至少包含前端入口未切换、外部代理未切换、生产流量未切换、旧入口退役审批未完成、集中配置未接入和持久化审计未接入。该 evidence 不得返回真实外部代理配置、前端构建变量、token、Cookie、内部签名、本地用户目录、环境变量、节点密钥、真实终端命令或真实文件路径。
