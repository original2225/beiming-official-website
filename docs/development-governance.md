...
本轮五个 core 独立 Maven 入口退役必须单独闭环。只允许逐个明确路径删除五个 core 的 `pom.xml`、独立 Spring Boot 启动类和独立 `application.yml`，不得删除目录，不得删除模块源码。完成后只能运行 `backend/unified-backend-service/pom.xml` 的统一后端回归，不能再运行五个 core 的独立 Maven 命令作为当前入口。
