...
`.local-docs/` 只保存不需要上传的本地资料，包括开发手册、阶段计划、本地测试记录、AI 对话交接文档、临时分析、调研草稿、排障记录、个人工作笔记和微服务开发指导文档。`.local-docs/` 必须被 `.gitignore` 忽略，不得提交到 GitHub。
...
每个微服务或模块正式开发前，必须先根据 `docs/requirements.md`、`docs/system-design.md`、本文件和该模块相关正式资料编写或补齐正式 API 文档。API 文档必须放在 `docs/`，并明确接口路径、方法、认证要求、权限点、请求字段、响应字段、错误码、分页规则、幂等规则、状态流转、失败降级、审计要求和验收口径。没有正式 API 文档时，不得进入测试用例编写或编码。
