  ...
  "data": {
    ...
    "behaviorCoverageByModule": [
      ...
      },
      {
        ...
        "completeBehaviorContractRoutesVerifiedTotal": 41,
        "pendingBehaviorContractRoutesTotal": 0,
        "behaviorCoverageStatus": "COMPLETE_BEHAVIOR_CONTRACTS",
        "pendingBehaviorCategories": []
      },
      ...
      },
      {
        ...
        "contract": "docs/contracts-changelog.md",
        "routesTotal": 23,
        "routeSignatureRoutesVerifiedTotal": 23,
...
`mvn -f backend/engagement-core-service/pom.xml test` 必须覆盖本文档三个自有接口和四个模块继承过来的全部契约测试。第三批旧服务清理后，相关回归基线为 `engagement-core-service`、`business-core-service`、`admission-core-service` 和 `unified-backend-service`，不得为了测试恢复 `community-service`、`activity-service`、`calendar-service`、`changelog-service` 或已退役的 `api-gateway-service` Maven 入口。
