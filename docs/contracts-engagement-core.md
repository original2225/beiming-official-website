...
| `completeBehaviorContractRoutesVerifiedTotal` | integer | 是 | 已完成全部行为契约验证的业务路由总数。当前为 `0`，代表路由和路由签名测试不得计入完整行为契约。 |
| `pendingBehaviorContractRoutesTotal` | integer | 是 | 仍需补齐完整行为契约验证的业务路由总数。当前为 `149`。 |
...
```json
{
  ...
  "data": {
    ...
    "pendingBehaviorContractRoutesTotal": 149,
